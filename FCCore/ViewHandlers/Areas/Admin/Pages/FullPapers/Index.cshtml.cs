...
namespace FCBEM24.Areas.Admin.Pages.FullPapers
{
    ...
    public class IndexModel : IReadPageModel<Paper>
    {
        ...
        public async Task<IActionResult> OnPostApprove()
        {
            ...
            try
            {
                ...
                string fileContents = System.IO.File.ReadAllText(Path.Combine(environment.WebRootPath, PathSource.MailContent, FileName.FullpaperAccept));
                string subject = string.Format(System.IO.File.ReadAllText(Path.Combine(environment.WebRootPath, PathSource.MailContent, FileName.FullpaperAcceptSubject)), $"{ProjectName} 20{ProjectYear}");
        ...
        public async Task<IActionResult> OnPostRejectAsync()
        {
            ...
            try
            {
                ...
                mimeEntity2.ContentId = MimeUtils.GenerateMessageId();
                mimeEntity3.ContentId = MimeUtils.GenerateMessageId();
                string fileContents = System.IO.File.ReadAllText(Path.Combine(environment.WebRootPath, PathSource.MailContent, FileName.FullpaperReject));
                string subject = string.Format(System.IO.File.ReadAllText(Path.Combine(environment.WebRootPath, PathSource.MailContent, FileName.FullpaperRejectSubject)), $"{ProjectName} 20{ProjectYear}");
                string htmlMessage = string.Format(fileContents, $"{ProjectName} 20{ProjectYear}", mimeEntity1.ContentId, mimeEntity2.ContentId, mimeEntity3.ContentId,
                    paper.PaperIDText, paper.ManuscriptTitle);
                builder.HtmlBody = htmlMessage;
                await iEmailSender.SendEmailAsync(author.Email, subject, builder);
        ...
        public IActionResult OnPostExcel()
        {
            ...
            {
                var sheet = package.Workbook.Worksheets.Add(SubmissionType.FullPaper);
                ...
                //sheet.Cells[1, 20].Value = "IdStatus";
                ...
                for (int i = 0; i < data1.Count; i++)
                {
                    ...
                    for (int j = 0; j < ab.Authors.Count; j++)
                    {
                        ...
                        {
                            sheet.Cells[j + 1, 17].Value = "Author";
                        }
            ...
            stream.Position = 0;
            var fileName = $"FullPaper_{DateTime.Now.ToString("yyyyMMddHHmmss")}.xlsx";
