...
namespace FCCore.Areas.Client.Pages.Papers
{
    ...
    public class CreateModel(UserManager<User>? userManager, IWebHostEnvironment environment, ILogger logger, IEmailSender iEmailSender, DatabaseContext context, IConfiguration configuration) : IWritePageModel<Paper>(userManager, environment, context, configuration)
    {
        ...
        public bool IsExpiredSubmission { get; set; }

        public virtual void CheckExpired()
        {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            try
            {
                ...
                if (Input.Keywords.Split(',').Length is < 3 or > 5 || Input.Keywords.Split(',').Any(a => a.Trim().Length < 2))
                {
                    StatusMessage = new StatusMessage("Type 3-5 keywords here, at least 2 characters for each key, separated by commas.", false).ToJSon();
                    SubmissionTypes = new SelectList(FCConstantsHelpers.SubmissionTypeSelectList, "Value", "Name", Input.Submission);
                    return Page();
                }
            ...
            catch (Exception ex)
            {
                ...
                logger.LogError(ex, "Create Paper error");
                return Page();
            }
        }
